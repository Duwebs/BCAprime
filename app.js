...
function card(r){const id=r.title.replace(/\W/g,'');const saved=state.saved.includes(id);const up=didUpvote(id);const sTitle=escHtml(r.title);const sSubject=escHtml(r.subject||'Community upload');const sCollege=r.college==='all'?'All colleges':escHtml((colleges.find(c=>c[0]===r.college)||['','College'])[1]);const sUploader=r.uploader?escHtml(r.uploader):'';const dlTitle=escJsStr(r.title);const isPending=r.status==='pending';const cnt=getCounts(id);const views=((cnt&&cnt.v)||0);const downloads=((typeof r.downloads==='number')?r.downloads:0)+((cnt&&cnt.d)||0);const isAdmin=(r.role==='admin'||r.uploaderRole==='admin');const avatar=isAdmin?(r.uploaderAvatar||'/assets/logo.png'):(r.uploaderAvatar||uploaderSmallAvatar(r.uploader||'S'));const saveBtn=`<button class="rc-icon ${saved?'on':''}" aria-label="Save resource" onclick="toggleSave('${id}')" title="Save"><i class="fa-${saved?'solid':'regular'} fa-bookmark"></i></button>`;const shareBtn=`<button class="rc-icon" aria-label="Share" onclick="shareResource('${dlTitle}')" title="Share"><i class="fa-solid fa-share-nodes"></i></button>`;const roleBadge=isAdmin?`<span class="rc-role rc-role-admin"><i class="fa-solid fa-circle-check"></i> Admin</span>`:`<span class="rc-role">Contributor</span>`;const fileExt=(r.fileName&&(r.fileName.match(/\.(\w+)$/)||[])[1])?escHtml(r.fileName.match(/\.(\w+)$/)[1].toUpperCase()):(r.type==='pyq'?'PYQ':'Notes');return `<article class="resource${isPending?' pending-resource':''}" data-id="${id}"><div class="rc-top"><div class="rc-top-left"><span class="badge">${r.type==='pyq'?'PYQ':'Notes'}</span><span class="rc-file">${fileExt}</span></div><div class="rc-top-actions">${saveBtn}${shareBtn}</div></div>${isPending?`<span class="rc-pending"><i class="fa-solid fa-clock"></i> Under review</span>`:''}<h3 class="rc-title">${sTitle}</h3><div class="rc-subject"><span class="rc-subject-badge"><i class="fa-solid fa-book-open"></i> ${sSubject}</span><span class="rc-sem-badge"><i class="fa-solid fa-layer-group"></i> Sem ${(r.sem!=null&&r.sem!=='')?escHtml(String(r.sem)):''}</span></div><div class="rc-meta"><span><i class="fa-solid fa-building-columns"></i>${sCollege}</span>${r.date?`<span><i class="fa-regular fa-clock"></i>${escHtml(r.date)}</span>`:''}</div><div class="rc-uploader"><img class="rc-uploader-avatar" src="${avatar}" alt="${sUploader||'BCAPrime'}" width="24" height="24">${sUploader?`<span class="rc-uploader-name">${sUploader}</span>`:''}${roleBadge}</div><div class="rc-stats"><button type="button" class="rc-stat rc-like${up?' on':''}" title="${up?'Unlike':'Like'}" onclick="toggleUpvote('${id}')"><i class="fa-${up?'solid':'regular'} fa-heart"></i><b>${upvoteDisplay(r)}</b> ${up?'Liked':'Likes'}</button><span class="rc-stat" title="Views"><i class="fa-regular fa-eye"></i><b id="rcv-${id}">${views}</b> Views</span><span class="rc-stat" title="Downloads"><i class="fa-solid fa-download"></i><b id="rcd-${id}">${downloads}</b> Downloads</span></div><div class="resource-actions"><button class="view read" onclick="readResource('${id}')"><i class="fa-solid fa-book-open"></i> Read</button><button class="download" onclick="download('${dlTitle}')"><i class="fa-solid fa-download"></i> Download</button></div></article>`}
    ...
    async function submitAccessAuth(event){event.preventDefault();if(!firebaseApp){$('accessAuthMessage').textContent='Firebase is not configured.';return}const email=$('accessAuthEmail').value.trim();const password=$('accessAuthPassword').value;if(accessAuthMode==='signup'&&!checkPasswordMatch(password,$('accessAuthConfirm'),$('accessAuthMessage')))return;$('accessAuthMessage').textContent='Working...';try{if(accessAuthMode==='signup'){await startPasswordSignup({name:$('accessAuthName').value.trim(),email,password,messageId:'accessAuthMessage'});return}await firebase.auth().signInWithEmailAndPassword(email,password);accountSession=firebase.auth().currentUser;if(await ensureVerified(accountSession)){sessionStorage.removeItem('bca-guest-mode');renderGreeting();resumeRestrictedAction()}}catch(error){$('accessAuthMessage').textContent=error.message;return}}
    ...
    async function sendOtpAndOpen(email,messageId){const url=getAuthApiUrl('sendOtp');const fallback=async()=>{pendingOtpApproval=false;const u=firebase.auth().currentUser;try{if(u)await u.sendEmailVerification()}catch(e){}showVerifyEmail(email,'We could not reach the verification service — we sent you a verification link instead.')};if(!url){fallback();return}const token=await firebaseIdToken();if(!token){fallback();return}setOtpStatus('Sending your code…','');try{const res=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({idToken:token,email})});const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data.error||'Could not send your verification code.');openOtpModal(email);startOtpCountdown(12*60)}catch(error){fallback()}}
    ...
    async function signInWithProvider(provider,messageId='accountMessage'){if(!firebaseApp){$(messageId).textContent='Firebase is not configured.';return}try{if(provider==='google'){await firebase.auth().signInWithPopup(new firebase.auth.GoogleAuthProvider());const user=firebase.auth().currentUser;if(user)sendGoogleWelcome(user)}}catch(error){if(error.code==='auth/popup-closed-by-user'||error.code==='auth/cancelled-popup-request')return;$(messageId).textContent=error.message}}
    let gateMode='signup';
    function setGateMode(mode){gateMode=mode;$('gateAccountSubmit').textContent=mode==='signup'?'Sign up':'Login';$('gateAccountSwitch').textContent=mode==='signup'?'Already have an account? Login':'Need an account? Sign up';$('gateAccountMessage').textContent='';$('gateNameLabel').hidden=mode!=='signup';$('gateConfirmLabel').hidden=mode!=='signup';if(mode==='signup'){$('gateAccountName').setAttribute('required','');$('gateAccountConfirm').setAttribute('required','')}else{$('gateAccountName').removeAttribute('required');$('gateAccountConfirm').removeAttribute('required')}}
    async function submitGateAccount(event){event.preventDefault();if(!firebaseApp){$('gateAccountMessage').textContent='Firebase is not configured.';return}const email=$('gateAccountEmail').value.trim();const password=$('gateAccountPassword').value;if(gateMode==='signup'&&!checkPasswordMatch(password,$('gateAccountConfirm'),$('gateAccountMessage')))return;$('gateAccountMessage').textContent='Working...';try{if(gateMode==='signup'){await startPasswordSignup({name:$('gateAccountName').value.trim(),email,password,messageId:'gateAccountMessage'});return}await firebase.auth().signInWithEmailAndPassword(email,password);accountSession=firebase.auth().currentUser;if(await ensureVerified(accountSession)){sessionStorage.removeItem('bca-guest-mode');showAuthenticatedApp()}}catch(error){$('gateAccountMessage').textContent=error.message;return}}
    ...
    async function saveProfileToAccount(){
      ...
      try{await supabaseClient.from('user_profiles').upsert({uid,email,college:state.college||'all',semester:sem,is_email_verified:!!(accountSession&&isVerifiedUser(accountSession)),updated_at:new Date().toISOString()},{onConflict:'uid'})}catch(e){}
    }
    ...
    async function submitAccount(event){event.preventDefault();if(!firebaseApp){$('accountMessage').textContent='Firebase is not configured.';return}const email=$('accountEmail').value.trim();const password=$('accountPassword').value;if(accountMode==='signup'&&!checkPasswordMatch(password,$('accountConfirm'),$('accountMessage')))return;try{if(accountMode==='signup'){await startPasswordSignup({name:$('accountName').value.trim(),email,password,messageId:'accountMessage'});return}await firebase.auth().signInWithEmailAndPassword(email,password);accountSession=firebase.auth().currentUser;if(await ensureVerified(accountSession)){sessionStorage.removeItem('bca-guest-mode');renderGreeting();renderAccount();toast('Account connected')}else{renderAccount()}}catch(error){$('accountMessage').textContent=error.message;return}}
